...
/datum/mod_link/proc/get_other()
	...
	if(!link_call)
		return
	return link_call.holocaller == src ? link_call.receiver : link_call.holocaller
...
/datum/mod_link/proc/call_link(datum/mod_link/called, mob/user)
	...
	alert.holocaller_ref = WEAKREF(src)
...
//the 'caller' var is used in jps.dm so we're naming it 'holocaller' instead of 'caller'
/datum/mod_link_call
	/// The MODlink that is calling.
	var/datum/mod_link/holocaller
...
/datum/mod_link_call/New(datum/mod_link/caller, datum/mod_link/receiver)
	holocaller.link_call = src
	receiver.link_call = src
	src.holocaller = holocaller
	src.receiver = receiver
	var/mob/living/holocaller_mob = holocaller.get_user_callback.Invoke()
	ADD_TRAIT(holocaller_mob, TRAIT_IN_CALL, REF(src))
...
/datum/mod_link_call/Destroy()
	var/mob/living/holocaller_mob = holocaller.get_user_callback.Invoke()
	if(!QDELETED(holocaller_mob))
		REMOVE_TRAIT(holocaller_mob, TRAIT_IN_CALL, REF(src))
	var/mob/living/receiver_mob = receiver.get_user_callback.Invoke()
	if(!QDELETED(receiver_mob))
		REMOVE_TRAIT(receiver_mob, TRAIT_IN_CALL, REF(src))
	STOP_PROCESSING(SSprocessing, src)
	clear_visuals()
	holocaller.link_call = null
...
/datum/mod_link_call/proc/can_continue_call()
	return holocaller.frequency == receiver.frequency && holocaller.can_call_callback.Invoke() && receiver.can_call_callback.Invoke()

/datum/mod_link_call/proc/make_visuals()
	var/holocaller_visual = holocaller.make_visual_callback.Invoke()
	var/receiver_visual = receiver.make_visual_callback.Invoke()
	holocaller.get_visual_callback.Invoke(receiver_visual)
	receiver.get_visual_callback.Invoke(holocaller_visual)

/datum/mod_link_call/proc/clear_visuals()
	holocaller.delete_visual_callback.Invoke()
	receiver.delete_visual_callback.Invoke()

/proc/call_link(mob/user, datum/mod_link/calling_link)
	if(!calling_link.frequency)
		return
	var/list/holocallers = list()
	for(var/id in GLOB.mod_link_ids)
		...
		if(!link.can_call_callback.Invoke())
			continue
		holocallers["[link.holder] ([id])"] = id
	if(!length(holocallers))
		calling_link.holder.balloon_alert(user, "no targets on freq [calling_link.frequency]!")
		return
	var/chosen_link = tgui_input_list(user, "Choose ID to call from [calling_link.frequency] frequency", "MODlink", holocallers)
	if(!chosen_link)
		return
	calling_link.call_link(GLOB.mod_link_ids[holocallers[chosen_link]], user)
...
/atom/movable/screen/alert/modlink_call
	...
	var/datum/weakref/holocaller_ref
...
/atom/movable/screen/alert/modlink_call/Click(location, control, params)
	...
	if(usr != owner)
		return
	var/datum/mod_link/holocaller = holocaller_ref.resolve()
	var/datum/mod_link/receiver = receiver_ref.resolve()
	if(!holocaller || !receiver)
		return
	if(holocaller.link_call || receiver.link_call)
		return
	var/list/modifiers = params2list(params)
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		end_message = "call denied!"
		owner.clear_alert("[REF(holocaller)]_modlink")
		return
	end_message = "call accepted"
	new /datum/mod_link_call(holocaller, receiver)
	owner.clear_alert("[REF(holocaller)]_modlink")
...
/atom/movable/screen/alert/modlink_call/Destroy()
	...
	if(!user || !holocaller)
		return ..()
	holocaller.holder.balloon_alert(user, end_message)
	return ..()

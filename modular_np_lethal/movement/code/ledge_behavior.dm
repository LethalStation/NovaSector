...
/mob/living/proc/recover_from_ledge(list/unsafe_turfs, channel = 20 SECONDS, channel_cancel = 18 SECONDS)
	if (stat == CONSCIOUS && !IsSleeping() && !IsUnconscious())
		...
		if (LAZYLEN(safe_turfs) >= 1) //we have at least 1 safe turf we can try to grasp onto
			...
			if (do_after(src, channel, salvation))
				...
				if (!pulledby)
					forceMove(the_chasm)
					visible_message(span_warning("A look of horror briefly crosses [src]'s features as their grip loosens, then they are gone, sailing down into the abyss..."), span_userdanger("Your gut lurches in horror as your grip works its way loose, and then you are falling... <i>falling...</i>"))
					return FALSE
